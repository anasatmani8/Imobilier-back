...
public class ImobilierRest {
	...
	/*
	 * @PostMapping("/upload/image") public ResponseEntity<Void>
	 * uplaodImage(@RequestParam("files") MultipartFile[] multipartFiles,
	 * 
	 ...
	 * 
	 * boolean isExist = new
	 * File("C:\\Users\\Admin\\Desktop\\Etude\\JEE S5\\Imobilier-back\\src\\main\\FilesP"
	 * ).exists(); if (!isExist) { new
	 * File("C:\\Users\\Admin\\Desktop\\Etude\\JEE S5\\Imobilier-back\\src\\main\\FilesP"
	 * ).mkdir(); System.out.println("--------------- mkdir"); }
	 * 
	 * try { System.out.println("Files liste"); for (MultipartFile file :
	 * multipartFiles) {
	 * System.out.println("file Name : "+file.getOriginalFilename());
	 * System.out.println("file Size : "+file.getSize());
	 * System.out.println("file Type : "+file.getContentType());
	 * System.out.println("------------------------------------"); save(file); if
	 * (file != null) {
	 * ImageRepo.save(Image.builder().imobilier(imobilier).name(file.
	 * getOriginalFilename())
	 * .type(file.getContentType()).image(ImageUtility.compressImage(file.getBytes()
	 * )).build()); }
	 * 
	 * } return new ResponseEntity<Void>(HttpStatus.OK); } catch (Exception e) {
	 ...
	 * File serverFilename = new
	 * File("C:\\Users\\Admin\\Desktop\\Etude\\JEE S5\\Imobilier-back\\src\\main\\FilesP"
	 * +File.separator+newFileName); System.out.println(serverFilename); try {
	 * System.out.println("image"); FileUtils.writeByteArrayToFile(serverFilename,
	 * file.getBytes());
	 * 
	...
	/*
	 * if (file2 != null) {
	 * /*ImageRepo.save(Image.builder().imobilier(imobilier).name(file2.
	 * getOriginalFilename())
	 * .type(file2.getContentType()).image(ImageUtility.compressImage(file2.getBytes
	 * ())).build());
	 * 
	 * } }
	 */
	/*private String save(MultipartFile file) {
		try {
			...
			java.nio.file.Path path = Paths
					.get("C:\\Users\\Admin\\Desktop\\Etude\\JEE S5\\Imobilier-back\\src\\main\\FilesP\\" + fileName);
	...
	public ResponseEntity<String> addImobilier(@RequestPart("imobilier") Map<String, String> requestMap,
	...
	/*
	 * @GetMapping(path="/achatDetails/{id}") ResponseEntity<List<Optional<?>>>
	 * getAchatDetails(@PathVariable int id){
	 * 
	 * try { return imobilierService.getAchatDetail(id); } catch (Exception ex) {
	 * ex.printStackTrace(); } return new ResponseEntity<>(new ArrayList<>(),
